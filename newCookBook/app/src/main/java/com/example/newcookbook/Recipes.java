...
public class Recipes extends AppCompatActivity {
   ...
   /* public void removeItem(int position) {
    ...
    public void createList() {
        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                child = dataSnapshot.child(NAME_USER).child(TYPE_FOOD).getChildrenCount();
                for(long i = 0 ; i < child ; i++){

                    if(dataSnapshot.child(NAME_USER).child(TYPE_FOOD).child(i+"").child("url").getValue().toString().equals("")) {
                        mList.add(new CardRecipes(
                                dataSnapshot.child(NAME_USER).child(TYPE_FOOD).child(i + "").child("name").getValue().toString(),
                        ...
                        mList.add(new CardRecipes(
                                dataSnapshot.child(NAME_USER).child(TYPE_FOOD).child(i + "").child("name").getValue().toString(),
                                dataSnapshot.child(NAME_USER).child(TYPE_FOOD).child(i + "").child("url").getValue().toString()
                        ));
                    }

                }

                Toast.makeText(Recipes.this, "successfully", Toast.LENGTH_SHORT).show();
            ...
            public void onCancelled(DatabaseError error) {
                ...
                Toast.makeText(Recipes.this, "failed", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public void movToCard(int position){
        ...
        intent.putExtra("typeFood",TYPE_FOOD);
